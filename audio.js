/* ==========================================
   HARDBEAT PRO - CORE AUDIO (DIRT FIXED)
   ========================================== */
const audioCtx = new (window.AudioContext || window.webkitAudioContext)();

// 1. CRÉATION DE L'EFFET "DIRT" (BITCRUSHER) - CORRIGÉ
const bitCrusherNode = audioCtx.createScriptProcessor(4096, 1, 1);
let currentBits = 16; // 16 bits = CD quality (Clean)

bitCrusherNode.onaudioprocess = function(e) {
    const input = e.inputBuffer.getChannelData(0);
    const output = e.outputBuffer.getChannelData(0);
    
    // Calcul de la résolution (Step)
    // Si on est en 16 bits, step est grand (65536). Si on est en 4 bits, step est petit (16).
    const step = Math.pow(2, currentBits); 
    
    // Si la qualité est haute (> 15 bits), on ne touche à rien pour économiser le CPU
    if (currentBits >= 15) {
        for (let i = 0; i < input.length; i++) {
            output[i] = input[i];
        }
    } else {
        // Sinon on applique l'effet "escalier" (Bitcrushing)
        for (let i = 0; i < input.length; i++) {
            // On multiplie, on arrondit (ça casse les détails), on re-divise
...
// --- API WINDOW MISE À JOUR ---

window.updateGlobalDirt = function(val) {
    // val est entre 0 (Clean) et 100 (Dirty)
    // On convertit ça en Bits :
    // 0% -> 16 bits (CD)
    // 100% -> 2 bits (Atari buggé)
    
    let dirtPercent = parseFloat(val);
    // Formule inversée : Plus le % est haut, plus les bits sont bas
    let bits = 16 - (dirtPercent / 100) * 14; 
...
// DRUMS & SYNTH FUNCTIONS (INCHANGÉES)
...
console.log("Audio Engine : DIRT V2 (CORRIGÉ) - READY.");
